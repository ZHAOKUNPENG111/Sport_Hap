...
def aicoach(path, sports,aaa):
    ...
    holistic.close()
    os.kill(int(pbzc_pid), signal.SIGTERM)
    return sports
...
# @app.route('/video_feed')
# def video_feed():
#     return Response(aicoach("./video/mypbzc.mp4", "plank"),mimetype='multipart/x-mixed-replace; boundary=frame')
...
def video_feed():
    ...
    return Response(aicoach(-1, "plank",int(args)),mimetype='multipart/x-mixed-replace; boundary=frame')



# if __name__ == '__main__':
#     aicoach("./test1.mp4", "plank")
