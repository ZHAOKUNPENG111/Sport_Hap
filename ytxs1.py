...
def video_stream(source,num_):
    global chinUp_cnt, up_complete 
    droid.ttsSpeak("引体向上运动现在开始")
    chinup_pid = get_current_process_id()
    print("chinup_pid2222222222222 = ",chinup_pid)

    # 读取视频
    save_path = './outputvideo/chinup.avi'
    cap = cvs.VideoCapture(source)
    cap1 = cv2.VideoCapture('/home/Aisports/image/Aicoach.png')
        ...
        while True:
            ...
            if results.pose_landmarks is not None:
                ...
                if pose != "NAN":
                    if pose == _PUSHUP:
                        # 在图像中输出俯卧撑以及次数
                        cv2.putText(bg_img, pose + ":" + str(int(pushUp_cnt / 2)), (200, 120), cv2.FONT_HERSHEY_SIMPLEX, 4,
                                    (48, 48, 255), 4)                   
                    if pose == _CHINUP:                    
                        # 在图像中输出引体向上以及次数以及完成度
                        if chinUp_cnt_changed != -1:
                            ...
                            if (chinUp_cnt / 2) >= num_:
                                chinup_pid = get_current_process_id()
                                print("chinup_pid2222222222222 = ",chinup_pid)
                                print("检测结束")
                                cv2.destroyAllWindows()
                                os.kill(int(chinup_pid), signal.SIGTERM)
                                break
...
def video_feed():
    ...
    return Response(video_stream(-1,int(args)),mimetype='multipart/x-mixed-replace; boundary=frame')
